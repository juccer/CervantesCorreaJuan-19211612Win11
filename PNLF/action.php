...
if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
    ...
    if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
        $SqlInsertAlumno = ("INSERT INTO pnlf(
            namefull,
            cedula,
            sexo,
            section,
...
if($metodoAction == 2){
    ...
    if (!empty($_FILES["foto"]["name"])){
        ...
        if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
            ...
            //Muevo la foto a mi directorio.
        if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
            $updateFoto = ("UPDATE pnlf SET foto='$nuevoNameFoto' WHERE id='$idAlumno' ");
...
if($metodoAction == 3){
    ...
    $SqlDeleteAlumno = ("DELETE FROM pnlf WHERE  id='$idAlumno'");
